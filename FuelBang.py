...
class FuelBang:
    ...
    def smoothing_relative(self, input_data: DataFrame, max_row: int, max_col: int,
                           process_params: RunParameters.ProcessParameters):
        """
        ...
        for row in range(max_row - 1, 0, -1):
            for col in range(max_col - 1, 1, -1):  # omit the zero column
                ...
                val_rpm = 0 if weight_rpm == 0 else (
                        (neighbour_rpm / process_params.smoothing_factor) * weight_rpm)
    ...
    def find_limits(self, input_data: DataFrame, process_params: RunParameters.ProcessParameters):
        """
    ...
    def process_zero_and_smooth(self, input_data: DataFrame, process_params: RunParameters.ProcessParameters):
        """
        ...
        :return: Nothing (in place modification of input_data)
        """
        max_row, max_col = self.find_limits(input_data, process_params)

        # insert zeroes
        input_data.iloc[1:max_row, 1:max_col] = 0

        # smoothing
        if process_params.smoothing_algorithm is self.RunParameters.ProcessParameters.Smoothing.Relative:
            self.smoothing_relative(input_data, max_row, max_col, process_params)
        elif process_params.smoothing_algorithm is self.RunParameters.ProcessParameters.Smoothing.RelativeLinear:
            self.smoothing_relative_linear(input_data, max_row, max_col, process_params)
        elif process_params.smoothing_algorithm is self.RunParameters.ProcessParameters.Smoothing.Linear:
            self.smoothing_linear(input_data, max_row, max_col, process_params)
        else:
            pass

    def process_multiply(self, input_data: DataFrame, process_params: RunParameters.ProcessParameters):
        """
        EXPERIMENTAL
        :param input_data: The fuel table to be processed
        :param process_params: Processing parameters
    ...
    def run(self):
        ...
        # process
        self.process_zero_and_smooth(data_l1, self.params.process_params)
        self.process_zero_and_smooth(data_l2, self.params.process_params)
...
def get_params():
    ...
    # detect algorithm
    if args["--linear"]:
        algorithm = FuelBang.RunParameters.ProcessParameters.Smoothing.Linear
    elif args["--relative"]:
        algorithm = FuelBang.RunParameters.ProcessParameters.Smoothing.Relative
    elif args["--relativeLinear"]:
        algorithm = FuelBang.RunParameters.ProcessParameters.Smoothing.RelativeLinear
    else:
    ...
    process_params = FuelBang.RunParameters.ProcessParameters(full_fuel_rpm=int(args["--ff_rpm"]),
